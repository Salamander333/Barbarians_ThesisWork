...
namespace Barbarians.Services
{
    public class ArenaService : IArenaService
    {
        ...
        public async Task<string> AttackOpponent(string attackerName, string opponentName)
        {
            ...
            var opponentDamage = _db.Weapons.Where(s => s.UserId == opponentId && s.IsBroken == false).OrderByDescending(x => x.Damage).FirstOrDefault() == null ? 0 : _db.Weapons.Where(s => s.UserId == opponentId && s.IsBroken == false).OrderByDescending(x => x.Damage).FirstOrDefault().Damage;
