...
namespace Barbarians.Services
{
    public class ArenaService : IArenaService
    {
        ...
        public async Task<string> AttackOpponent(string attackerName, string opponentName)
        {
            ...
            var attackerDamage = _db.Weapons.Where(s => s.UserId == attackerId && s.IsBroken == false).OrderByDescending(x => x.Damage).FirstOrDefault() == null ? 10 : _db.Weapons.Where(s => s.UserId == attackerId && s.IsBroken == false).OrderByDescending(x => x.Damage).FirstOrDefault().Damage;
            ...
            var opponentDamage = _db.Weapons.Where(s => s.UserId == opponentId && s.IsBroken == false).OrderByDescending(x => x.Damage).FirstOrDefault() == null ? 10 : _db.Weapons.Where(s => s.UserId == opponentId && s.IsBroken == false).OrderByDescending(x => x.Damage).FirstOrDefault().Damage;
